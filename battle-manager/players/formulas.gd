...
static func calculate_damage(attacker, target, attack:CharacterAbilities) -> int:
	...
	match GlobalBattleSettings.Global_Damage_Calc_Type:
		GlobalBattleSettings.Damage_Calc_Type.PKMN:
			damage = max(1, int((((((2.0 * attacker.exp_node.char_level) / 5.0 + 2) * attack.number_value * (atk / def)) / 50.0) + 2) * element_wheel(attacker.stats.element, target.stats.element)))
		GlobalBattleSettings.Damage_Calc_Type.DRGNQST:
			damage = max(1, int(((atk - (def / 2.0) + (((atk - (def/2.0) + 1.0) * randf_range(0, 255)) / 256.0)) / 4) * (attack.number_value / 50.0) * element_wheel(attacker.stats.element, target.stats.element)))
		_:
